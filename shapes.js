...
function Shape(state, x, y, w, h, fill, img, scale, text) {
  "use strict";
  ...
  this.img = img || 'no_image';
  this.scale = scale || false;
  this.inner_text = text || 'no_text';
}

// Draws this shape to a given context
Shape.prototype.draw = function(ctx, optionalColor) {
  "use strict";
  var i, cur, half, my_shape;
  my_shape = this;

  if (this.img !== "no_image") {
    this.img.onload = function() {
      ctx.drawImage(this ,my_shape.x ,my_shape.y, my_shape.w, my_shape.h);
      this.style.display = 'none';
    };
    ctx.drawImage(this.img ,my_shape.x ,my_shape.y, my_shape.w, my_shape.h);
    this.img.style.display = 'none';

  }
  if (this.inner_text !== "no_text") {

  }
  if (this.img === "no_image" && this.inner_text === "no_text"){
...
function update_labels() {
  ...
  if (state !== undefined) {
    if (state.selection !== null) {
      $('#selected_item_x').val(state.selection.x);
      $('#selected_item_y').val(state.selection.y);
      $('#selected_item_width').val(state.selection.w);
      $('#selected_item_height').val(state.selection.h);
    }
    else{
      $('#selected_item_x').val(0);
      $('#selected_item_y').val(0);
      $('#selected_item_width').val(0);
      $('#selected_item_height').val(0);
    };
  };
}

function update_shape_x() {
  "use strict";
  if (state !== undefined) {
    if (state.selection !== null) {
      state.selection.x = parseInt($('#selected_item_x').val());
      state.valid = false;
    };
  };
}

function update_shape_y() {
  "use strict";
  if (state !== undefined) {
    if (state.selection !== null) {
      state.selection.y = parseInt($('#selected_item_y').val());
      state.valid = false;
    };
  };
}

function update_shape_w() {
  "use strict";
  if (state !== undefined) {
    if (state.selection !== null) {
      state.selection.w = parseInt($('#selected_item_width').val());
      state.valid = false;
    };
  };
}

function update_shape_h() {
  "use strict";
  if (state !== undefined) {
    if (state.selection !== null) {
      state.selection.h = parseInt($('#selected_item_height').val());
      state.valid = false;
    };
  };
}
...
function init() {
  "use strict";
  state = new CanvasState(document.getElementById('canvas'));
  $('#create_item').attr('onclick', 'create_shape()');
  $('#selected_item_x').attr('onclick', 'update_shape_x()')
  $('#selected_item_y').attr('onclick', 'update_shape_y()')
  $('#selected_item_width').attr('onclick', 'update_shape_w()')
  $('#selected_item_height').attr('onclick', 'update_shape_h()')
  state.canvas.addEventListener('mousedown', update_labels);
  state.canvas.addEventListener('mousemove', update_labels);
  state.canvas.addEventListener('mouseup', update_labels);
  var img = new Image();
  img.src = 'avatar.jpg';


  // add a large green rectangle
  state.addShape(new Shape(state, 0, 0, 60, 65, 'rgba(0,205,0,0.7)'));
  // add a green-blue rectangle
  state.addShape(new Shape(state, 240, 120, 40, 40, 'rgba(2,165,165,0.7)'));
  // add a smaller purple rectangle
  state.addShape(new Shape(state, 5, 60, 25, 25, 'rgba(150,150,250,0.7)'));
  state.addShape(new Shape(state, 20, 25, 50, 50, "a", img));
}
